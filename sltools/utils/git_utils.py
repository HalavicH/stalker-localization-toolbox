...
def is_allowed_to_continue(path, allow_no_repo, allow_dirty, allow_not_tracked):
    ...
    if not allow_no_repo:
        if not is_git_available():
            console.print(
                _tr("Git is not available on this system. It's vital to have files in a git repo to prevent potential file damage. Please install Git."))
            return False
    ...
    if not allow_no_repo:
        try:
            ...
            log.warning(_tr("'It seems that there's no git repository here. Skipping the file processing. "))
        ...
        if not allow_not_tracked:
            ...
            if not tracked_files_output:
                log.warning(_tr("'File %s' is not tracked by git. Skipping the file processing.") % path)
...
def log_ignore_option(option):
    log.always(_tr("Note: If you sure you won't break anything, you can process this path anyway using %s") % cf_cyan(option))
